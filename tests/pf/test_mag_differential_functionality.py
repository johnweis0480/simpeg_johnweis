...
def mesh():
    ...
    dhx, dhy, dhz = 75., 75., 75.  # minimum cell width (base mesh cell width)
...
def test_recievers(mesh):
    ...
    components_2 = ['by','tmi']
    rxLoc_1 = PF.magnetics.receivers.Point(np.c_[utils.mkvc(ccy.T), utils.mkvc(ccx.T), utils.mkvc(ccz.T)],
                                         components=components_1)
    rxLoc_2 = PF.magnetics.receivers.Point(np.c_[utils.mkvc(ccy.T), utils.mkvc(ccx.T), utils.mkvc(ccz.T+20)],
                                         components=components_2)
    inducing_field = [55000.0, 60., 90.]

    srcField_1 = PF.magnetics.sources.UniformBackgroundField([rxLoc_1], inducing_field[0], inducing_field[1],
                                                               inducing_field[2])
    survey_1 = PF.magnetics.survey.Survey(srcField_1)

    srcField_2 = PF.magnetics.sources.UniformBackgroundField([rxLoc_2], inducing_field[0], inducing_field[1],
                                                             inducing_field[2])
    survey_2 = PF.magnetics.survey.Survey(srcField_2)

    srcField_all = PF.magnetics.sources.UniformBackgroundField([rxLoc_1,rxLoc_2], inducing_field[0], inducing_field[1],
                                                           inducing_field[2])
    ...
    rvec = np.random.randn(mesh.n_cells*4)*.001
