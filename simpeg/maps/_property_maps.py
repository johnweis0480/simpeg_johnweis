...
class ESusMap(IdentityMap):
    ...
    def __init__(self, mesh=None, nP=None,ambient_field_magnitude=None, **kwargs):
        super().__init__(mesh=mesh, nP=nP, **kwargs)
        if ambient_field_magnitude is None or not isinstance(ambient_field_magnitude, (float, int)):
            raise TypeError("ambient_field_magnitude must be a float (or int convertible to float) and cannot be None")
